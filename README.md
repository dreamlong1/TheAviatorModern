...
```
  🖱️ 鼠标移动
      │
      ▼
  handleMouseMove()  ──►  mousePos { x, y }
                                │
                                ▼
                         updatePlane()  ──►  飞机位置阻尼插值 + Roll/Pitch 姿态
  ...
  │  │                                                           │  │
  │  │  updateDistance() ──► 升级判断 ──► 速度提升               │  │
  │  │                                                           │  │
  │  │  sea.mesh.rotation   ──►  大海自转（营造飞行感）           │  │
  │  │  sky.mesh.rotation   ──►  云层缓缓漂移                    │  │
  │  │                                                           │  │
  │  │  CoinsHolder.spawnCoins()    ──►  按距离阈值刷新药丸       │  │
  │  │  EnnemiesHolder.spawnEnnemies() ──►  按距离阈值刷新障碍物  │  │
  │  │                                                           │  │
  ...
  │  └───────────────────────────────────────────────────────────┘  │
  │                                                                 │
  │  Sea.moveWaves()              ──►  海面顶点波浪动画             │
  │  Pilot.updateHairs()          ──►  飞行员头发飘动               │
  │  ParticlesHolder.updateParticles() ──►  碰撞粒子衰减消散        │
  │                                                                 │
  │  renderer.render(scene, camera)  ──►  输出到 Canvas             │
  └─────────────────────────────────────────────────────────────────┘
```
